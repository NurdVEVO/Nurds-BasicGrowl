...
namespace com.basicGrowl.Services
{
    internal sealed partial class SeekerToneServices
    {
            ...
            float dt)
        {
            ...
            var cagedClip = GetOrLoadClip(folder, cagedFile);
            if (cagedClip != null)
            {
            ...
            {
                var uncagedClip = GetOrLoadClip(folder, uncagedFile);
                if (uncagedClip == null)
                {
        ...
        private void ApplyEnvTone(string folder, string envFile, float pitch, float envVol, float dt)
        {
            ...
            var envClip = GetOrLoadClip(folder, envFile);
            if (envClip == null)
            {
        ...
        private AudioClip GetOrLoadClip(string folder, string fileName)
        {
            ...
            _loading.Add(key);
            _plugin.StartCoroutine(LoadWav(folder, fileName, key));
            return null;
        }

        private IEnumerator LoadWav(string folder, string fileName, string cacheKey)
        {
