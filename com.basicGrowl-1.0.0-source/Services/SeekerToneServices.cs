...
namespace com.basicGrowl.Services
{
    internal sealed partial class SeekerToneServices
    {
        ...
        private const int LosLayerMask = 64;      // matches game IRSeeker linecast
        ...
        private static readonly PropertyInfo NetworkHQProperty =
            typeof(Unit).GetProperty("NetworkHQ", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public void Tick()
        {
            ...
            // IR weapon gate
            if (prefab.GetComponentInChildren<IRSeeker>(true) == null)
            {
                ClearActiveProfile();
                ResetSeekerState(keepDir: true);
                StopTone();
            ...
            if (SeekerProfileRegistry.TryReloadIfChanged())
            {
                _activeProfile = null;
            ...
            {
                if (!SeekerProfileRegistry.TryGetByPrefab(prefabName, out var prof) || prof == null)
                {
                    ClearActiveProfile();
                    ResetSeekerState(keepDir: true);
                    StopTone();
                ...
                _activeMissileForRange = ResolveActiveMissileForRange(prefab);
