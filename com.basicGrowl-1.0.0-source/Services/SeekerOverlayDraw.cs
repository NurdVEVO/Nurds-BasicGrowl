...
namespace com.basicGrowl.Services
{
    internal sealed class SeekerOverlayDraw : MonoBehaviour
    {
        ...
        private int _lastHandledCameraId;
        ...
        private void OnEnable()
        {
            ...
            Camera.onPostRender += OnLegacyPostRender;
            Plugin.Log?.LogInfo("[Overlay] Enabled. Hooks: endCameraRendering + onPostRender");
        }
        ...
        {
            DrawForCamera(cam, "SRP");
        }

        private void OnLegacyPostRender(Camera cam)
        {
            DrawForCamera(cam, "Legacy");
        }

        private void DrawForCamera(Camera cam, string source)
        {
            if (cam == null)
            {
                _lastCameraName = "<null>";
                _lastCameraId = 0;
                _lastCallbackSource = source;
            ...
            if (_lastHandledFrame == frame && _lastHandledCameraId == camId)
                return;

            _lastHandledFrame = frame;
            _lastHandledCameraId = camId;

            if (!_loggedFirstCallback)
            {
                Plugin.Log?.LogInfo($"[Overlay] First callback via {source} cam='{cam.name}' type={cam.cameraType}");
            ...
            if (_lineMaterial == null || _seekerToneServices == null)
            {
                SetSkipReason("missing material/service");
                return;
            }

            if (!ShouldRenderOnCamera(cam))
                return;

            if (!_seekerToneServices.TryGetOverlayState(out var originWorld, out var seekerDirWorld))
            {
                SetSkipReason("overlay state hidden");
                return;
            }

            if (!TryProjectRing(cam, originWorld, seekerDirWorld, out var centerX, out var centerY, out var radiusPx, out var reason))
            {
            ...
            if (!_loggedFirstDraw)
            {
                Plugin.Log?.LogInfo($"[Overlay] First draw success cam='{cam.name}' source={source} center=({centerX:F1},{centerY:F1}) radius={radiusPx:F1}");
                _loggedFirstDraw = true;
            }
        }

        private bool ShouldRenderOnCamera(Camera cam)
        {
            if (cam.cameraType != CameraType.Game)
            {
                SetSkipReason($"camera type filtered ({cam.cameraType})");
                return false;
            }

            if (cam.targetTexture != null)
            {
                SetSkipReason("camera targetTexture set");
                return false;
            }
            ...
            if (cam != targetCam)
            {
                if (preferredCam != null)
                    SetSkipReason($"not preferred camera ({PreferredCameraName})");
                else if (hasMode)
                    SetSkipReason($"cockpit mode fallback main camera (main={targetCam.name})");
                else
                    SetSkipReason($"not fallback main camera (main={targetCam.name})");
            ...
            out string reason)
        {
            ...
            const float sampleDistance = 2000f;
            Vector3 centerWorld = originWorld + seekerDirWorld * sampleDistance;
            ...
            centerY = Screen.height - centerScreen.y;
